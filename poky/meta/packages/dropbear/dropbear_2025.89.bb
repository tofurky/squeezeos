...
SRC_URI = "https://matt.ucc.asn.au/dropbear/releases/dropbear-${PV}.tar.bz2 \
	...
	file://avoid-unused-declaration-with-unix-socket-disabled.patch;patch=1 \
	file://fix-svr-authpubkey-whitespace.patch;patch=1 \
	file://fix-off-by-one-server-fail-count.patch;patch=1 \
	file://limit-server-number-of-public-key-queries.patch;patch=1 \
	file://server-disallow-signal-with-forced-command.patch;patch=1 \
	file://server-disallow-signal-without-dropping-privs.patch;patch=1 \
...
CFLAGS_prepend = " -I. -std=gnu99 "
LD = "${CC}"

SBINCOMMANDS = "dropbear dropbearkey dropbearconvert"
BINCOMMANDS = "dbclient ssh scp"
EXTRA_OEMAKE = 'MULTI=1 SCPPROGRESS=1 PROGRAMS="${SBINCOMMANDS} ${BINCOMMANDS}"'
EXTRA_OECONF += "--srcdir=src"
...
pkg_postrm_append () {
  ...
  if [ -f "${sysconfdir}/dropbear/dropbear_ed25519_host_key" ]; then
        rm ${sysconfdir}/dropbear/dropbear_ed25519_host_key
  fi
